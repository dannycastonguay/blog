...
class LinkedInPlatform(SocialMediaPlatform):
    ...
    def _get_user_id(self) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.create_error_result("LinkedIn", "Unable to get user ID", error_msg)
                raise ValueError(error_msg)
        except Exception as e:
            error_msg = f"Error getting LinkedIn user ID: {str(e)}"
            self.create_error_result("LinkedIn", "Error getting user ID", error_msg)
            raise ValueError(error_msg)
    ...
    def _post_text_only(self, content: str) -> Dict[str, Any]:
        ...
        if response.status_code == 201:
            ...
            return self.create_success_result(
                "personal_profile", content, post_id, post_url
            )
        else:
            raise ValueError(
    ...
    def _post_with_media(self, content: str, media_files: List[str]) -> Dict[str, Any]:
        ...
        for media_id in media_ids:
            ...
            if mime_type and mime_type.startswith("video/"):
                media_entity = {"status": "READY", "media": media_id}
            else:
                media_entity = {
                    "status": "READY",
                    "media": media_id,
    ...
    def _upload_media(self, media_file: str) -> Optional[str]:
        ...
        try:
            ...
            response = requests.post(init_url, headers=headers, json=register_data)
            ...
            if upload_response.status_code not in (200, 201):
                raise ValueError(
                    f"Failed to upload media: {upload_response.status_code} - {upload_response.text}"
                )
