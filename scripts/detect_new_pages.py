...
def get_changed_files_from_pr():
    ...
    base_sha = run_command(
        f"gh pr view {pr_number} --json baseRefOid --jq '.baseRefOid'"
    )
    if not base_sha:
        return {"added": [], "modified": [], "deleted": []}
    ...
    for line in output.split("\n"):
        ...
        check_command = (
            f"git cat-file -e {base_sha}:{file_path} 2>/dev/null || echo 'not_exists'"
        )
...
def detect_changed_posts(
    base_ref=None, head_ref=None, posts_dir="_posts", pr_mode=False
):
    """
    ...
    return {"added": new_posts, "modified": modified_posts, "deleted": deleted_posts}
    ...
    if any(changed_posts.values()):
        ...
        if "GITHUB_OUTPUT" in os.environ:
            ...
            with open(os.environ["GITHUB_OUTPUT"], "a") as f:
                ...
                all_changed = (
                    changed_posts["added"]
                    + changed_posts["modified"]
                    + changed_posts["deleted"]
                )
